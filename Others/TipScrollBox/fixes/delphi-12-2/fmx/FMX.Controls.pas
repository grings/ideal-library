...
{ TControl }
  ...
  private
	...
	// -------------------------------------------------------------------------
    // [iPub - github.com/viniciusfbb] - 19/04/2021 - Delphi 10.4
    // Best control of the mouse operations propagation over parent tree
    // proposed by Stephane in https://quality.embarcadero.com/browse/RSP-24397
{$IFDEF DELPHI_FIXES} // ---------------------------------------------------
    FPropagateMouseOperations: Boolean;
{$ENDIF} // ----------------------------------------------------------------    	
  ...
  protected
    ...
    // -------------------------------------------------------------------------
    // [iPub - github.com/viniciusfbb] - 16/08/2020 - Delphi 10.4
    // Reset the root focus when one parent (in the parent tree) of the focused
    // control is setting nil.
{$IFDEF DELPHI_FIXES} // ---------------------------------------------------
    procedure DoRootChanging(const ANewRoot: IRoot); override;
{$ENDIF} // ----------------------------------------------------------------
...
{$ENDIF} // ----------------------------------------------------------------
    ...
    // proposed by Stephane in https://quality.embarcadero.com/browse/RSP-24397
{$IFDEF DELPHI_FIXES} // ---------------------------------------------------
    property PropagateMouseOperations: Boolean read FPropagateMouseOperations write FPropagateMouseOperations;
{$ENDIF} // ----------------------------------------------------------------
...
begin
  ...
  // proposed by Stephane in https://quality.embarcadero.com/browse/RSP-24397
{$IFDEF DELPHI_FIXES} // -----------------------------------------------------
  FPropagateMouseOperations := True;
{$ENDIF} // ------------------------------------------------------------------
end;
...
{$IFDEF DELPHI_FIXES} // -------------------------------------------------
      for var I: Integer := 1 to FUpdating do
...
// control is setting nil.
{$IFDEF DELPHI_FIXES} // -------------------------------------------------------
procedure TControl.DoRootChanging(const ANewRoot: IRoot);
begin
  inherited;
  if FIsFocused and (Root <> nil) then
    Root.SetFocused(nil);
end;
{$ENDIF} // --------------------------------------------------------------------
...
begin
  ...
  // https://quality.embarcadero.com/browse/RSP-24397
  {$IFDEF DELPHI_FIXES} // -----------------------------------------------------
  if PropagateMouseOperations and (FParentControl <> nil) then
    FParentControl.ChildrenMouseEnter(Self);
  {$ENDIF} // ------------------------------------------------------------------
end;
...
begin
  ...
  {$IFDEF DELPHI_FIXES} // -----------------------------------------------------
  if PropagateMouseOperations and (FParentControl <> nil) then
    FParentControl.ChildrenMouseLeave(Self);
  {$ENDIF} // ------------------------------------------------------------------	
end;
...
begin
  ...
  {$IFDEF DELPHI_FIXES} // -----------------------------------------------------
  if PropagateMouseOperations and (FParentControl <> nil) then
    FParentControl.ChildrenMouseDown(Self, Button, Shift, X, Y);
  {$ENDIF} // ------------------------------------------------------------------
...
begin
  ...
  {$IFDEF DELPHI_FIXES} // -----------------------------------------------------
  if PropagateMouseOperations and (FParentControl <> nil) then
    FParentControl.ChildrenMouseMove(Self, Shift, X, Y);
  {$ENDIF} // ------------------------------------------------------------------	
end;
...
begin
  ...
  {$IFDEF DELPHI_FIXES} // -----------------------------------------------------
  if PropagateMouseOperations and (FParentControl <> nil) then
    FParentControl.ChildrenMouseUp(Self, Button, Shift, X, Y);
  {$ENDIF} // ------------------------------------------------------------------
  if FPressed then
  begin
    FPressed := False;
